 ...
 *		 If the endpoint is IN, the endpoint register will be set to accept the
 *		 request.
...
UINT  _ux_dcd_dwc3_transfer_request(UX_DCD_DWC3 *dcd_dwc3, UX_SLAVE_TRANSFER *transfer_request)
{
   ...
   ULONG events;
	...
	/* Adjust the data pointer.  */
        if ((endpoint -> ux_slave_endpoint_descriptor.bmAttributes & UX_MASK_ENDPOINT_TYPE) == UX_BULK_ENDPOINT)
	    {
		   ...
		   /* Sending 0 length packet  */
		    ...
		    status_cmd = _ux_utility_event_flags_get(&BULKIN_BULKOUT_FLAG, UX_BULKIN_EVENT | UX_DISCONNECT_EVENT, UX_OR_CLEAR, &events,
    ...
    {
	if(transfer_request -> ux_slave_transfer_request_requested_length > endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize)
	{
	    transfer_request -> ux_slave_transfer_request_requested_length =   endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize;
	}
	...
	TX_RESTORE
        if ((Ept->ux_dcd_dwc3_ed_status & UX_DCD_EP_BUSY) != 0U)
        {
			...
			UX_DCD_EP_TRB      *TrbTempNext;
                        ...
                        TrbTempNext->BufferPtrHigh  = ((ULONG)BufferPtr >>
                                                                 16U) >> 16U;
                                ...
                                ULONG *BufferPtr, ULONG BufferLen)
{
        ...
        default:
         ...
         TrbPtr->Ctrl |= ( UX_DCD_DWC3_TRB_CTRL_CSP
			| UX_DCD_DWC3_TRB_CTRL_IOC
                        | UX_DCD_DWC3_TRB_CTRL_ISP_IMI
                        | UX_DCD_DWC3_TRB_CTRL_HWO);
