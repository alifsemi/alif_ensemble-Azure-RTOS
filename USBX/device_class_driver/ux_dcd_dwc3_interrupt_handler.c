   ...
   dcd_dwc3->test_mode = 0;
    ...
    while(event_buffer->count > 0)
    {
		...
		ULONG	Index;
			...
			case UX_DCD_EVENT_RESET:
			    ...
			    for (Index = 0U; Index <
		             (dcd_dwc3->NumInEps + dcd_dwc3->NumOutEps);
		                        Index++)
			    {
		...
		case UX_DCD_EVENT_CONNECT_DONE:
			...
			//GUSB2CFG initilized in controller initiliaze routine
    ...
    else
    {
	    /* Endpoint specific events */
	     ...
	     /* Device specific events. For now 7 events registered*/
		           ...
		           case EP0_SETUP_PHASE:
			           ...
			           dcd_dwc3 -> ep0state = (enum UX_DCD_EP0_STATE)EP0_NRDY_STATUS;
                               ...
                               else
                               {
                                  ...
                                  dcd_dwc3 -> ep0_expect_in = !!(Ctrl->bRequestType &
                                                            UX_REQUEST_IN);
                               }
	    ...
	    else
	    {
	       ...
	       transfer_request =  &endpoint -> ux_slave_endpoint_transfer_request;
               ...
               switch (dwc3_register)
	       {
	           /* Below are endpoint other than 0 interrupts will be handled later */
