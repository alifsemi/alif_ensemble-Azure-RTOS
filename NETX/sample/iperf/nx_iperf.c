...
NXD_ADDRESS peer_ip_address;
    ...
    else if (nx_iperf_ctrl_info.TestStatus == 2)
    {
        ...
        if ((!nx_iperf_ctrl_info.ThroughPut) && (nx_iperf_ctrl_info.StartTime))
        {
            ...
            /* Calculate the run time and Throughput(Kbps).  */
            ...
            nx_iperf_ctrl_info.ThroughPut = (float) (nx_iperf_ctrl_info.BytesTxed + nx_iperf_ctrl_info.BytesRxed) / nx_iperf_ctrl_info.RunTime * NX_IP_PERIODIC_RATE / 125.0;
        }
...
UINT       length = 0;
    ...
    status += htmlwrite(resp_packet_ptr, tdtag, sizeof(tdtag) - 1);
    status += htmlwrite(resp_packet_ptr, fonttag, sizeof(fonttag) - 1);
    status += htmlwrite(resp_packet_ptr, "Throughput(Kbps): ", sizeof("Throughput(Kbps): ") - 1);
...
ULONG       port;
    ...
    /* Calculate the test time and Throughput(Kbps).  */
    ctrlInfo_ptr -> RunTime = tx_time_get() - ctrlInfo_ptr -> StartTime;
    ctrlInfo_ptr -> ThroughPut = (float) (ctrlInfo_ptr -> BytesRxed) / ctrlInfo_ptr -> RunTime * NX_IP_PERIODIC_RATE / 125.0;
...
NXD_ADDRESS server_ip;
    ...
    /* Calculate the test time and Throughput(Kbps).  */
    ctrlInfo_ptr -> RunTime = tx_time_get() - ctrlInfo_ptr -> StartTime;
    ctrlInfo_ptr -> ThroughPut = (float) (ctrlInfo_ptr -> BytesTxed) / ctrlInfo_ptr -> RunTime * NX_IP_PERIODIC_RATE / 125.0;
...
NXD_ADDRESS source_ip_address;
    ...
    while (tx_time_get() < expire_time)
    {
        ...
        NX_CHANGE_ULONG_ENDIAN(packetID);
...
NXD_ADDRESS  server_ip;
    ...
    NX_CHANGE_ULONG_ENDIAN(payload_ptr -> udp_id);
...
long       udp_id;
    ...
    /* Calculate the test time and Throughput.  */
    ctrlInfo_ptr -> RunTime = tx_time_get() - ctrlInfo_ptr -> StartTime;
    ctrlInfo_ptr -> ThroughPut = (float) (ctrlInfo_ptr -> BytesTxed) / ctrlInfo_ptr -> RunTime * NX_IP_PERIODIC_RATE / 125000;
