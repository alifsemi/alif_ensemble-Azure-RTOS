...
//#define SECONDARY_SLOT_OFFSET  0x80020000
#define SECONDARY_SLOT_OFFSET  0x20000
...
{
	printf("nx_azure_iot_adu_agent_driver entry!!\n");
}
*/
void  nx_azure_iot_adu_agent_driver(NX_AZURE_IOT_ADU_AGENT_DRIVER *driver_req_ptr)
{
...
int32_t buf_size = 0;
    ...
    switch (driver_req_ptr -> nx_azure_iot_adu_agent_driver_command)
    {
        ...
        {
        	printf("case : NX_AZURE_IOT_ADU_AGENT_DRIVER_INITIALIZE\n");
            /* Process initialize requests.  */

        	int32_t ret = MRAM_drv->Initialize();
            if (ret)
            {
                printf("Flash driver init FAIL!\n");
            ...
            if (ret)
            {
                printf("Flash driver power control FAIL!\n");
                driver_req_ptr -> nx_azure_iot_adu_agent_driver_status = NX_AZURE_IOT_FAILURE;
            }
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_PREPROCESS:
        {
        	...
        	printf("case : NX_AZURE_IOT_ADU_AGENT_DRIVER_PREPROCESS\n");
            break;
        }
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_WRITE:
        {
        	...
        	printf("case : NX_AZURE_IOT_ADU_AGENT_DRIVER_WRITE\n");
            uint32_t offset = SECONDARY_SLOT_OFFSET + driver_req_ptr->nx_azure_iot_adu_agent_driver_firmware_data_offset;
            //printf("Write offset = 0x%x, ptr = %p, size = %d\n",
            //       offset,
            ...
            printf("Offset --> %x\n",offset);
            //printf("Total bytes need to be written 2 --> %d\n",driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_size);
            int32_t ret = MRAM_drv->ProgramData(offset,
                                                driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_data_ptr,
                                                driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_data_size);
            buf_size = driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_data_size;
            uint8_t buff_RX[buf_size];
            int32_t ret_value;
            ret_value = MRAM_drv->ReadData(offset, buff_RX,buf_size);

            cmp = memcmp(driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_data_ptr, buff_RX, buf_size);
            if(cmp != 0)
            {
                printf("\r\n Error: MRAM write-read failed ...\n");
                while(1)
                {
                }
            }

           // printf("Actually written bytes are --> %d\n", ret);
            if (ret != driver_req_ptr -> nx_azure_iot_adu_agent_driver_firmware_data_size)
            {
                printf("Flash driver write FAIL!\n");
                driver_req_ptr -> nx_azure_iot_adu_agent_driver_status = NX_AZURE_IOT_FAILURE;
            }
            break;
        }
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_INSTALL:
        {
        	...
        	printf("case : NX_AZURE_IOT_ADU_AGENT_DRIVER_INSTALL\n");
            break;
        }
        ...
        case NX_AZURE_IOT_ADU_AGENT_DRIVER_APPLY:
        {
        	...
        	printf("case : NX_AZURE_IOT_ADU_AGENT_DRIVER_APPLY\n");
            break;
        }
        default:
        {
